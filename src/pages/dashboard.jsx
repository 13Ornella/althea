...
import {Bar, BarChart, CartesianGrid, Legend, ResponsiveContainer, Tooltip, XAxis, YAxis, Rectangle,PieChart, Pie, Sector, Cell} from "recharts";
...
useEffect(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/nbCandidature')
      .then((response) => {
        setNbCandidature(response.data);
      })
      .catch((error) => {
        console.error('Erreur' + error);
      });
  ...
  useEffect(() => {
    ...
    return(       
        <div className="bg-gray-50">
          <div className="flex">
            <Sidebar/>
           <ul className="flex gap-4 grid grid-cols-3"></ul>      
           <Flydash/>
...
<div class=" mt-6 gap-4 grid grid-cols-2 ">
            <div className="flex gap-2 bg-white rounded-sm p-4 flex-1 border-gray-200 shadow-md ronded-md h-24 ">
            <svg class="w-16 h-16 text-gray-800 dark:text-blue-950 border border-gray-100 bg-gray-100 rounded-full p-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
      ...
      {nbOffre.map((callout, index) => (
        <option key={index} value={callout.id}  className="text-sm font-semibold text-blue-950">
          {callout.name}
            ...
            <div  className="flex gap-2 bg-white rounded-sm p-4 flex-1 border-gray-200  items-center shadow-md rounded-md h-24">
            <svg class="w-16 h-16 text-gray-800 dark:text-teal-700 border border-gray-100 bg-gray-100 rounded-full p-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 20">
<path d="M16 1h-3.278A1.992 1.992 0 0 0 11 0H7a1.993 1.993 0 0 0-1.722 1H2a2 2 0 0 0-2 2v15a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V3a2 2 0 0 0-2-2Zm-3 14H5a1 1 0 0 1 0-2h8a1 1 0 0 1 0 2Zm0-4H5a1 1 0 0 1 0-2h8a1 1 0 1 1 0 2Zm0-5H5a1 1 0 0 1 0-2h2V2h4v2h2a1 1 0 1 1 0 2Z"/>
</svg>  
{Array.isArray(nbCandidature) && nbCandidature.map((callout, index) => ( 
  <span 
  key={index}
  className="font-bold text-2xl text-teal-700">
    
       Postulants: {callout.nbCandidature}
    </span>))}    
            </div>
            <Statistique/>
            <Barchart/>
...
<th className="p-4 text-sm font-semibold tracking-wide text-left">Nom</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Prenom</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Email</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Telephone</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Situation</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Sexe</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Domaine</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Niveau</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Experiences</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Adresse</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left">Voir</th>
<th className="p-4 text-sm font-semibold tracking-wide text-left ">Supprimer</th>
  </tr></thead>
  ...
  filteredCandidatures.map((callout, index) => (
    ...
    <td className="p-4 text-sm text-gray-600">{callout.prenom}</td>
    <td className="p-4 text-sm text-gray-600">{callout.email}</td>
    <td className="p-4 text-sm text-gray-600">{callout.telephone}</td>
    <td className="p-4 text-sm text-gray-600">{callout.situation}</td>
   ...
   <td className="p-4 text-sm text-gray-600"><Link to={`/entretien?id=${callout.id}`}>
   ...
   <td className="p-4 text-sm text-gray-600">
    <svg xmlns="http://www.w3.org/2000/svg" height="1.5em" viewBox="0 0 448 512"><path d="M135.2 17.7L128 
